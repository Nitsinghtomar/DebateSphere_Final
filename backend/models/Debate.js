...
const debateSchema = new mongoose.Schema({
  ...
  // 🔥 NEW: Analysis Integration Fields
  finishedAt: {
    type: Date,
    default: null
  },
...
debateSchema.index({ analysisStatus: 1 }); // 🔥 NEW: Index for analysis queries

// 🔥 FIXED: Virtual for duration with null check
...
// 🔥 FIXED: Virtual for participant count with safety check
...
// 🔥 NEW: Instance method to finish debate and prepare for analysis
debateSchema.methods.finishDebate = function(durationInSeconds) {
  this.status = 'finished';
...
// 🔥 NEW: Static method to find user debates with analysis status
debateSchema.statics.findUserDebatesWithAnalysis = function(userId) {
  return this.find({ 'participants.user': userId })
    .populate('participants.user', 'username profilePicture')
    .populate('analysisId')
    .sort({ createdAt: -1 });
